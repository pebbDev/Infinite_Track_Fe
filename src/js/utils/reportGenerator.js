...
class ReportGenerator {
     ...
     */    generatePDF(summaryData, period = 'all') {
        ...
        if (summaryData.report && summaryData.report.data && summaryData.report.data.length > 0) {
            const reportStartY = doc.lastAutoTable ? doc.lastAutoTable.finalY + 20 : 100;
            
            doc.setFontSize(14);
            doc.text('Attendance Report', 20, reportStartY);
            
            const reportColumns = ['Name', 'NIP/NIM', 'Role', 'Date', 'Check In', 'Check Out', 'Work Hour', 'Status', 'Work Category'];
            ...
            autoTable(doc, {
                ...
                columnStyles: {
                    0: { cellWidth: 20 }, // Name
                    1: { cellWidth: 15 }, // NIP/NIM
                    2: { cellWidth: 15 }, // Role
                    3: { cellWidth: 15 }, // Date
                    4: { cellWidth: 12 }, // Check In
                    5: { cellWidth: 12 }, // Check Out
                    6: { cellWidth: 12 }, // Work Hour
                    7: { cellWidth: 12 }, // Status
                    8: { cellWidth: 20 }  // Work Category
                }
            });
        }
    ...
    generateExcel(summaryData, period = 'all') {
        ...
        if (summaryData.summary) {
            ...
            // Style the header
            summarySheet['A1'] = { v: 'Attendance Summary Report', t: 's', s: { font: { bold: true, sz: 16 } } };
        ...
        if (summaryData.report && summaryData.report.data && summaryData.report.data.length > 0) {
            const reportData = [
                ['Name', 'NIP/NIM', 'Role', 'Email', 'Date', 'Check In', 'Check Out', 'Work Hour', 'Status', 'Work Category', 'Information', 'Notes'],
...
export const generatePDFReport = (summaryData, period) => reportGenerator.generatePDF(summaryData, period);
export const generateExcelReport = (summaryData, period) => reportGenerator.generateExcel(summaryData, period);
